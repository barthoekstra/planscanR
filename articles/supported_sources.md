...
- **Geometry.** Beyond the free-text *concelho* names in
  `municipalities`, each detail page’s *Localização* links to APA’s
  SNIAMB geoviewer, whose ArcGIS *ZoomToApp* service serves the project
  polygon. The handler queries it (`Referer`-gated, `outSR=4326`, Esri
  rings converted to GeoJSON in-package) and writes a sibling
  `<document_id>.geometry.geojson` in **EPSG:4326** (WGS84) with
  `geometry_path` / `geometry_crs` when a footprint exists. The geometry
  host is throttled separately via
  `getOption("planscanR.pt_geo_throttle_rate")`.
