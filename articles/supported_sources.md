...
Bulgaria, the Czech Republic, Croatia, Greece, and Iceland). There is no
shared API behind the scenes — each country gives us a different mix of
HTML pages, sitemaps, search endpoints, and undocumented JSON handlers,
and each one comes with its own quirks. This vignette describes what
each handler does, where it pulls data from, and what to expect (or
watch out for) when using it.
